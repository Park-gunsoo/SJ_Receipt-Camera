...
일본 영수증 촬영·조회 모바일 웹, PC 수정 화면과 공통 처리 백엔드. **본인/초대 테스터용**이며 일본어·한국어·영어를 지원합니다. 웹에서 목록 내 수정·저장, 삭제(휴지통 이동)·복원, 언어를 선택하는 엑셀 다운로드를 지원합니다. 영구 삭제는 후속 개발입니다.
...
- 각 행의 연필 버튼으로 날짜·거래처·적요·계정과목·금액·세금·결제 정보를 수정하고 저장합니다. 엑셀은 현재 검색 결과 전체 또는 전체 영수증에서 저장된 값만 내려받으며, 언어는 일본어/한국어/영어 중 선택합니다. 영수증 장부와 세율별 내역 2개 시트로 내보내며, 과다한 다운로드는 오류로 안내하고 일부만 잘라 내보내지 않습니다.
