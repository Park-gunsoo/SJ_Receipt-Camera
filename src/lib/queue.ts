...
export async function dispatchPending(receiptId?: string) {
  ...
  const eligible = { state: "PENDING" as const, receipt: { intakeState: "ACCEPTED" as const, deletedAt: null }, nextRunAt: { lte: new Date() }, AND: [{ OR: [{ enqueuedAt: null }, { enqueuedAt: { lt: stale } }] }, { OR: [{ kind: { not: "ARCHIVE" as const } }, { receipt: { pdfState: "SAVED" as const } }] }], ...(receiptId ? { receiptId } : {}) };
