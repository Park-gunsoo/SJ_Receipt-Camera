...
export function CategoryField({ value, onChange, classification }: { value: string; onChange: (value: string) => void; classification?: Classification | null }) {
  const { t, locale } = useLanguage();
  const [custom, setCustom] = useState(false);
  const isCustom = custom || Boolean(value && !accountCategoryNames.includes(value));
  return <div className="category-field span-two"><label>{t("勘定科目（候補）")}<select value={isCustom ? "__custom__" : value} onChange={event => {
    ...
    {isCustom && <label className="custom-category">{t("任意の勘定科目")}<input value={value} maxLength={80} onChange={event => onChange(event.target.value)} /></label>}
    {classification && <div className="category-suggestion"><div><Sparkles size={15} /><strong>{t("原本からの科目候補")}</strong></div>
    ...
    <small>{t("法人・個人共通の候補です。用途に合わせて修正できます。")}</small>
  </div>;
}
