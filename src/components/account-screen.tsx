...
export function AccountScreen() {
  ...
  return <section><div className="page-heading"><div><p className="eyebrow">{t("YOUR ACCOUNT")}</p><h1>{t("アカウントと接続")}</h1></div></div><p className="page-lead">{t("保存先と、データの取り扱い。")}</p>
    ...
    <div className="card" id="privacy"><div className="card-title"><ShieldCheck size={20} /><h2>{t("データの取り扱い")}</h2></div><p>{t("写真は非公開のサーバーに送信し、Google Cloud Visionで文字を読み取ります。PDFはあなたのGoogle Driveに保存します。")}</p><p>{t("テスト期間中、原本写真・読み取り結果は自動削除しません。未送信の写真はこの端末内に、アカウント別に一時保存します。")}</p><p>{t("ブラウザのデータを削除すると、未送信の写真が失われる場合があります。受付完了を確認するまでは、画面を開いたままお待ちください。")}</p><p>{t("編集・削除機能は準備中です。テストデータの削除は運営者に依頼してください。")}</p><small className="muted">{t("このバージョンは招待されたテスト利用者向けです。")}</small></div>
