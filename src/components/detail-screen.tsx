...
export function DetailScreen({ id }: { id: string }) {
  ...
  return <section><Link className="back-link" href="/m/receipts"><ArrowLeft size={18} />{t("履歴に戻る")}</Link><div className="page-heading"><div><p className="eyebrow">{t("RECEIPT DETAILS")}</p><h1>{t("レシート詳細")}</h1></div></div><Status receipt={receipt} />
    ...
    <div className="card"><h2>{t("読み取り内容")}<span className="small-tag">{t("未確定")}</span></h2><dl className="detail-fields">{fields.map(([label, value]) => <div key={label}><dt>{label}</dt><dd className={!value ? "unconfirmed" : ""}>{value || t("未確認")}</dd></div>)}</dl>{receipt.reviewReasons.length > 0 && <div className="review-reasons"><strong><Info size={15} />{t("確認が必要な項目")}</strong><ul>{receipt.reviewReasons.map(reason => <li key={reason}>{displayText(reason, locale)}</li>)}</ul></div>}</div>
    <div className="two-actions">{receipt.archiveState === "SAVED" && <a className="button secondary" href={`/api/receipts/${id}/pdf`} target="_blank" rel="noopener noreferrer"><FileText size={17} />{t("PDFを見る")}</a>}{receipt.driveUrl && <a className="button secondary" href={receipt.driveUrl} target="_blank" rel="noopener noreferrer">{t("Driveで開く")}<ExternalLink size={16} /></a>}</div><p className="footnote">{t("内容の編集機能は準備中です。")}<br />{t("この画面を見るだけでは、確認完了にはなりません。")}</p>
