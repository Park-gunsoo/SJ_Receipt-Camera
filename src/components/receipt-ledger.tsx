...
function LedgerFilters({ query, categories = [], onApply }: { query: LedgerQuery; categories?: string[]; onApply: (query: LedgerQuery) => void }) {
  ...
  return <form className="ledger-filters" onSubmit={submit}>
...
export function ReceiptLedger() {
  ...
  return <section className="receipt-ledger"><div className="ledger-page-heading"><div className="ledger-heading-title"><span className="ledger-heading-icon"><Table2 size={22} /></span><div><h1>{t("レシート帳簿")}</h1><p>{t("原本と読み取り内容を、ひとつの帳簿で。")}</p></div></div><button className="button ledger-refresh" disabled={loading} onClick={() => load()}><RefreshCw size={16} className={loading ? "spin" : undefined} />{t("更新")}</button></div>
    <LedgerFilters key={queryString} query={query} categories={data?.categories} onApply={navigate} />
    {!parsed.success && <p className="ledger-error" role="alert">{t("検索条件が正しくありません。条件をクリアしてください。")}</p>}
    {notice && <div className="ledger-notice" role="status">{t("ゴミ箱に移動しました")} <Link href="/web/trash">{t("ゴミ箱を開く")}</Link></div>}
    ...
    <div className="ledger-sheet" aria-busy={loading}><div className="ledger-sheet-toolbar"><div><strong>{t("検索結果")}</strong><span className="ledger-count">{loading ? "…" : t("{count}件", { count: data?.total ?? 0 })}</span></div><span>{t("取引先をクリックして詳細・修正")}</span></div>
      <div className="ledger-table-scroll" tabIndex={0} role="region" aria-label={t("レシート帳簿の表")}><table className="ledger-table"><caption className="visually-hidden">{t("レシート帳簿の表")}</caption><colgroup><col className="ledger-col-number" /><col className="ledger-col-date" /><col className="ledger-col-merchant" /><col className="ledger-col-category" /><col className="ledger-col-amount" /><col className="ledger-col-tax" /><col className="ledger-col-state" /><col className="ledger-col-storage" /><col className="ledger-col-actions" /></colgroup><thead><tr><th scope="col">No.</th><th scope="col" aria-sort={query.sort === "date" ? query.direction === "asc" ? "ascending" : "descending" : "none"}><button onClick={() => sort("date")}>{t("利用日")}{sortIcon("date")}</button></th><th scope="col" aria-sort={query.sort === "merchant" ? query.direction === "asc" ? "ascending" : "descending" : "none"}><button onClick={() => sort("merchant")}>{t("取引先・摘要")}{sortIcon("merchant")}</button></th><th scope="col">{t("勘定科目（候補）")}</th><th scope="col" className="amount-cell" aria-sort={query.sort === "amount" ? query.direction === "asc" ? "ascending" : "descending" : "none"}><button onClick={() => sort("amount")}>{t("合計金額")}{sortIcon("amount")}</button></th><th scope="col" className="amount-cell">{t("消費税の内訳")}</th><th scope="col">{t("読み取り状況")}</th><th scope="col">{t("ファイル")}</th><th scope="col">{t("操作")}</th></tr></thead>
      ...
      <div className="ledger-pagination"><span>{loading ? t("読み込んでいます…") : t("{total}件中 {from}–{to}件を表示", { total: data?.total ?? 0, from: data?.total ? (page - 1) * query.pageSize + 1 : 0, to: Math.min(page * query.pageSize, data?.total ?? 0) })}</span><div><label>{t("表示件数")}<select value={query.pageSize} onChange={event => navigate({ ...query, pageSize: Number(event.target.value), page: 1 })}>{[25, 50, 100].map(size => <option key={size} value={size}>{size}</option>)}</select></label><button aria-label={t("前のページ")} disabled={loading || page <= 1} onClick={() => navigate({ ...query, page: page - 1 })}><ChevronLeft size={17} /></button><span>{page} / {pages}</span><button aria-label={t("次のページ")} disabled={loading || page >= pages} onClick={() => navigate({ ...query, page: page + 1 })}><ChevronRight size={17} /></button></div></div>
