...
export function Shiba({ className = "", variant = "camera", sizes = "128px", preload = false, decorative = false }: {
  ...
  return <Image src={asset.src} alt={decorative ? "" : t(asset.alt)} width={asset.size} height={asset.size} sizes={sizes} preload={preload} className={`mascot mascot-${variant} ${className}`} />;
}
