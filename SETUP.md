...
현재 서비스에는 Google 로그인·선택형 Drive 백업·OCR·비공개 앱 PDF 보관과 배포 설정이 적용돼 있습니다. 아래는 같은 구성을 준비하거나 복구할 때 사용하는 절차이며, 최신 적용 상태는 CURRENT_STATUS.md를 확인하세요. 외부 설정이 없는 로컬 화면에서는 연동 성공을 가장하지 않습니다. 비밀키를 채팅에 붙여넣거나 Git에 커밋하지 마세요.
...
1. Chrome에서 HTTPS 주소를 열고 Google 로그인만 완료합니다. Drive 미연결 상태에서도 촬영·앱 PDF 조회가 가능해야 합니다. Drive 백업을 원하면 계정 화면에서 별도로 연결·활성화합니다.
2. 실제 영수증을 1장 촬영합니다. 별도 저장 버튼 없이 `送信中`이 나와야 합니다.
3. `受付完了` 후 화면을 잠그거나 브라우저를 닫습니다. 다시 열어 앱 PDF를 확인합니다. Drive 백업을 켠 경우 같은 영수증이 Drive에도 한 번만 저장되는지 확인합니다.
...
6. 업로드 응답을 끊은 뒤 재접속해 DB 영수증과 앱 PDF가 각 1개인지 확인합니다. 선택한 Drive 백업도 중복 생성되지 않아야 합니다.
7. worker 처리 중 재시작 후, lease 만료(최대 10분)와 Scheduler를 거쳐 복구되는지 확인합니다.
8. OCR 오류가 있어도 앱 PDF가 저장되어야 합니다. Drive 권한 취소·용량 부족은 백업 경고로 표시하고, 앱 촬영·분석·PDF 조회를 차단하지 않아야 합니다.
