...
Inline editing/XLSX export (2026-09-21): no migration. Inline edits use the same values/version/ReceiptRevision path as detail editing. Export reads saved active owner records in one repeatable-read snapshot with the same filter predicates as the ledger, excluding raw OCR/storage keys and all trash. No export rows or files are stored in the DB.
...
- App trash uses the existing Receipt.deletedAt field; no migration is required. Trash/restore increments version but leaves receipt values, raw OCR, original keys, Drive IDs, revisions and ownership intact. Only the dedicated owner-scoped trash list can read deleted summaries. Worker completion retains in-flight results while trashed; new claims and dispatch exclude deleted receipts.
