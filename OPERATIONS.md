...
Receipt.intakeState=ACCEPTED, acceptedAt 존재, 원본 GCS 객체/sha256/길이 검증 완료, ARCHIVE/OCR Job 영속 기록. 모바일 상태와 달리 로그에만 성공이 있거나 ID만 만든 것은 접수가 아닙니다.
...
- 자동 작업 시도는 기본 5회입니다. Drive 권한/용량 문제는 BLOCKED로 정지하고 사용자의 재연결 후 재개합니다. 한도 초과 OCR은 다음 JST 날짜부터 다시 시도합니다.
...
- 원본이나 사용자 Drive PDF를 장애 해결 목적으로 자동 삭제하지 않습니다. 삭제 요청은 현재 수동 운영입니다. 인증/소유권을 확인하고 서버 자료와 사용자 Drive 자료를 구분하여 사용자가 요청한 범위만 처리합니다.
